	...
	'tree-node': {
      ...
      v-if="tmp[layer].tooltip != ''"
			:text="(tmp[layer].isLayer) ? (
				player[layer].unlocked ? (tmp[layer].tooltip ? tmp[layer].tooltip : format(player[layer].points,3) + ' ' + tmp[layer].resource)
				: (tmp[layer].tooltipLocked ? tmp[layer].tooltipLocked : 'Reach ' + format(tmp[layer].requires,3) + ' ' + tmp[layer].baseResource + ' to unlock (You have ' + format(tmp[layer].baseAmount,3) + ' ' + tmp[layer].baseResource + ')')
			)
			: (
